...
const NOTES = {
  inventario: { what: "WCI tiene 3 zonas: bodega seca, cámara de frío, cocina de producción. Stock por zona con transferencias. Insumos se crean manualmente o desde Compras al recibir factura con producto nuevo. Lista maestra en Configuración.", poc: "Datos mock. En producción se conecta a supplies, warehouse_items, supply_movements existentes.", validate: ["¿Las 3 zonas cubren todos los espacios reales?", "¿Transferencias entre zonas — flujo claro?", "¿Merma valorizada como proceso diario o semanal?", "¿Conteo físico semanal o mensual?"], decision: "Cada zona = warehouse en Supabase. default_warehouse_id define destino por insumo.", roles: ["Gte. Operaciones (full)", "Bodeguero (edita)", "Jefe cocina local (ve su inventario en Prep)"] },
  compras: { what: "WCI compra a proveedores. Modelo: factura primero, recepción después (abastella). 3 niveles de ingreso de factura. Items de factura se mapean a insumos del sistema; si un producto no se reconoce, se puede crear el insumo y mapearlo en un solo paso.", poc: "SII automático, PDF+IA, manual son mocks. En producción: SII API, Claude API para PDF, formulario.", validate: ["¿OC → Factura → Recepción es el flujo correcto?", "¿Sugerencia automática de items bajo mínimo es útil?", "¿Mapeo de glosas del proveedor es necesario?", "¿Recepción sin factura se usa en la práctica?"], decision: "Costeo promedio ponderado se recalcula al recepcionar.", roles: ["Gte. Operaciones (full — crea OC)", "Bodeguero (recepciona)"] },
  ...
  experiencia: { what: "CRM reclamos con SLA, NPS propio, customer journeys múltiples por local (Mesa, Mesón, Delivery) con touchpoints y timeline, pautas de evaluación editables por la Enc. UX, evaluaciones y comparación entre locales, ranking compuesto.", poc: "ManyChat webhook clasifica DMs de IG y crea tickets automáticos en el CRM.", validate: ["¿Touchpoints por journey (mesa/mesón/delivery) cubren la operación real?", "¿SLA por severidad (4h/24h/48h) son realistas?", "¿Workflow reclamos tiene sentido?", "¿Pautas con pesos y tipos de respuesta son suficientes para el score?", "¿Ranking con score compuesto sirve?", "¿Falta algún canal de feedback?"], decision: "Reclamos recurrentes por touchpoint → ciclo Kaizen automático.", roles: ["Enc. UX (full)", "Gte. Operaciones (escala)", "Admin local (ve su local)"] },
...
const MODULES = [
  ...
  { cat: "Marca", color: B.purple, items: [
    ...
    { id: "experiencia", icon: "⭐", name: "Experiencia", p: 8, tabs: ["CRM", "NPS", "Customer Journey", "Ranking", "ManyChat"] },
  ]},
      ...
      <header style={{ background: B.primary, padding: "0 32px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 0 24px" }}>
          ...
          <div style={{ display: "flex", gap: 20, marginTop: 16 }}>
            {[{ l: "Módulos", v: allMods.length }, { l: "Procesos", v: allMods.reduce((s, m) => s + (m.p || 0), 0) }, { l: "Pendientes", v: 4 }].map(s => <div key={s.l}><div style={{ fontSize: 26, fontWeight: 900, color: B.accent }}>{s.v}</div><div style={{ fontSize: 11, color: "#ffffff70" }}>{s.l}</div></div>)}
          </div>
        </div>
      </header>
